...
//            a.recycle();
    ...
    protected void onSizeChanged(int width, int height, int oldw, int oldh) {
        ...
        if (width > height){
            radius = height/2f - 3 * px;
        }else{
            radius = width/2f - 3 * px;
        }
...
//		Log.d(TAG, "Height spec: " + MeasureSpec.toString(heightMeasureSpec));
    ...
    private void drawLegend(Canvas canvas){
        ...
        double halfCircumference = (2 * radius * Math.PI) / 8 * 6;
        double increments = 50;
        for(int i = 0; i <= 300; i += increments){
            circle.addCircle(centerX, centerY, radius, Path.Direction.CW);
            canvas.drawTextOnPath(String.format("%d", i + 900),
                    circle,
                    (float) (i*halfCircumference/300),
                    -30f,
                    scalePaint);
